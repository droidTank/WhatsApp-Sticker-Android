...
public class StickerPackValidator {
   ...
   public static void verifyStickerPackValidity(@NonNull Context context, @NonNull StickerPack stickerPack) throws IllegalStateException {
       ...
       // TODO: 3/3/19
        ...
        try {
            final byte[] bytes = StickerPackLoader.fetchStickerAsset(stickerPack.getIdentifier(), stickerPack.getTrayImageFile(), context.getContentResolver());
        ...
        for (final Sticker sticker : stickers) {
            validateSticker(context, stickerPack.getIdentifier(), sticker);
        }
    }
